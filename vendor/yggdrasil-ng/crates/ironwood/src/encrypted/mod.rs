...
use self::crypto::{CurvePrivateKey, GroupAuth, ed25519_private_to_curve25519};
...
impl EncryptedPacketConn {
    ...
    pub async fn count_lookup_targets(
        &self,
        dest: crate::crypto::PublicKey,
    ) -> (crate::crypto::PublicKey, usize) {
        ...
        for action in actions {
            match action {
                OutAction::SendToInner { dest, data } => {
                    tracing::debug!(
                        "encrypted_reader: sending {} bytes to inner (session msg)",
                        data.len()
                    );
                ...
                OutAction::Deliver { source, data } => {
                    tracing::debug!(
                        "encrypted_reader: delivering {} bytes from {:?}",
                        data.len(),
                        hex::encode(&source[..4])
                    );
