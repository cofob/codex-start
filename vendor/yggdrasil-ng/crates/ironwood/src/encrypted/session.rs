...
use super::crypto::{
    BOX_OVERHEAD, CurvePrivateKey, CurvePublicKey, GroupAuth, box_open, box_open_precomputed,
    box_seal, box_seal_precomputed, ed25519_public_to_curve25519, make_salsa_box, new_box_keys,
};
...
impl SessionInit {
    ...
    pub fn decrypt(
        ...
        let payload =
            box_open(ciphertext, 0, &from_box, our_curve_priv).map_err(|_| Error::Decode)?;
...
impl SessionInfo {
    ...
    pub fn new(current: CurvePublicKey, next: CurvePublicKey, seq: u64) -> Self {
...
fn encrypt_outside_lock(snap: &SendSnapshot, msg: &[u8]) -> Result<Vec<u8>, Error> {
    ...
    let ciphertext = box_seal_precomputed(&inner, snap.send_nonce, &snap.send_shared)
...
fn decrypt_outside_lock(
    snap: &RecvSnapshot,
    msg: &[u8],
) -> Result<(Vec<u8>, [u8; 32]), RecvAction> {
...
impl ConcurrentSessionManager {
    ...
    pub fn handle_data(
        ...
        match data[0] {
            ...
            SESSION_TYPE_TRAFFIC => self.handle_traffic(from, data, our_ed_priv),
            _ => Vec::new(),
        }
    }
    ...
    fn handle_traffic(
        ...
        let snap = {
            ...
            match info.recv_snapshot(data) {
                Ok(snap) => snap,
                Err(RecvSnapshotError::SendInit {
                    send_pub,
                    next_pub,
                    local_key_seq,
                }) => {
    ...
    fn handle_init(
        ...
        if let Some(session_arc) = existing {
            ...
            if let Ok(data) = ack_init.encrypt(
                our_ed_priv,
                from,
                SESSION_TYPE_ACK,
                self.group_auth.preimage(),
    ...
    fn buffer_and_init(
        ...
        if let Ok(data) = buf.init.encrypt(
            our_ed_priv,
            dest,
            SESSION_TYPE_INIT,
            self.group_auth.preimage(),
        ) {
            actions.push(OutAction::SendToInner { dest: *dest, data });
        }
...
mod tests {
    ...
    fn group_auth_preimage_matches_go() {
        ...
        let expected =
            hex::decode("16ccb8396ed9c53e85c369adb6b3befcb275a75bc185bcbd5f01f4c6fa54fee8")
                .unwrap();
        assert_eq!(
            GroupAuth::new(b"shared-password").preimage(),
            expected.as_slice()
        );
    ...
    fn init_encrypt_decrypt() {
        ...
        let encrypted = init
            .encrypt(&priv_a, &pub_b, SESSION_TYPE_INIT, &[])
            .unwrap();
    ...
    fn ack_encrypt_decrypt() {
        ...
        let encrypted = init
            .encrypt(&priv_a, &pub_b, SESSION_TYPE_ACK, &[])
            .unwrap();
