...
impl RouterAnnounce {
    ...
    pub fn check(&self) -> bool {
        ...
        Crypto::verify(&self.key, &bs, &self.sig) && Crypto::verify(&self.parent, &bs, &self.psig)
    }
...
pub(crate) enum RouterAction {
    ...
    PathNotifyCallback { key: PublicKey },
}
...
impl Router {
    ...
    pub fn do_maintenance(&mut self) -> Vec<RouterAction> {
        ...
        if self.last_status_log.elapsed() >= Duration::from_secs(60) {
            ...
            tracing::debug!(
                ...
                self.peers.len(),
                self.infos.len(),
                self.pathfinder.paths.len(),
                n_broken,
                self.pathfinder.rumors.len(),
                unresponded
            );
        }
        ...
        if self.last_refresh.elapsed() >= self.router_refresh {
            tracing::debug!(
                "Router refresh timer fired ({}s elapsed), triggering new SigReq cycle",
        ...
        for dest in self.pathfinder.rumors_needing_retry() {
            ...
            if self
                .pathfinder
                .should_throttle_rumor_retry(&xform, self.path_throttle)
            {
                continue;
            }
    ...
    fn blooms_maintenance(&mut self) -> Vec<RouterAction> {
        ...
        let parent_map: HashMap<PublicKey, PublicKey> =
            self.infos.iter().map(|(k, v)| (*k, v.parent)).collect();

        let to_send =
            self.blooms
                .do_maintenance(&self_key, &self_parent, &parent_map, &self.bloom_transform);
    ...
    pub fn add_peer(&mut self, entry: PeerEntry) -> Vec<RouterAction> {
        ...
        self.peers.get_mut(&key).unwrap().insert(entry.id, entry);
        ...
        if let Some(bloom) = self.blooms.get_send_bloom(&key) {
            actions.push(RouterAction::SendBloom { peer_id, bloom });
        }
    ...
    fn new_req(&self) -> SigReqState {
        ...
        let seq = self.infos.get(&self.crypto.public_key).map_or(0, |i| i.seq) + 1;
    ...
    pub fn handle_request_with_data(&self, peer: &PeerEntry, req: &wire::SigReq) -> RouterAction {
        let res_bytes = Self::sig_res_bytes_for_sig(
            &peer.key,
        ...
        if let Some(info) = self.infos.get(&ann.key) {
            ...
            match () {
                _ if info.seq > ann.seq => {
                    tracing::debug!(
                        "Announce REJECTED (old seq): key={:02x?} old_seq={} new_seq={}",
                        &ann.key[..8],
                        info.seq,
                        ann.seq
                    );
                ...
                _ if info.parent < ann.parent => {
                    tracing::debug!(
                        "Announce REJECTED (parent ordering): key={:02x?} old_parent={:02x?} new_parent={:02x?}",
                        &ann.key[..4],
                        &info.parent[..4],
                        &ann.parent[..8]
                    );
                ...
                _ => {
                    tracing::debug!(
                        "Announce REJECTED (nonce/default): key={:02x?} old_nonce={} new_nonce={}",
                        &ann.key[..8],
                        info.nonce,
                        ann.nonce
                    );
            ...
            tracing::debug!(
                "Announce NEW node: key={:02x?} parent={:02x?} seq={}",
                &ann.key[..4],
                &ann.parent[..8],
                ann.seq
            );
    ...
    pub fn handle_announce(
        ...
        if self.update(ann) {
            tracing::debug!(
                ...
                hex::encode(&ann.key[..8]),
                hex::encode(&ann.parent[..8]),
                ann.seq
            );
            if ann.key == self.crypto.public_key {
                tracing::debug!(
                    "Announce for self accepted, setting refresh=true (from peer {:?}, seq={})",
                    hex::encode(&ann.parent[..8]),
                    ann.seq
                );
                self.refresh = true;
    ...
    fn fix(&mut self) -> Vec<RouterAction> {
        ...
        if self.refresh || self.do_root1 || self.do_root2 || self_info_parent != best_parent {
            tracing::debug!(
                ...
                hex::encode(&best_parent[..8]),
                hex::encode(&best_root[..8]),
            );
            ...
            if let Some(res) = res {
                if best_root != self_key && self.use_response(&best_parent, &res) {
                    ...
                    tracing::debug!(
                        "Tree: adopted parent {} root {} coords={:?}",
                        hex::encode(&best_parent[..8]),
                        hex::encode(&best_root[..8]),
                        new_coords
                    );
                ...
                tracing::debug!(
                    "fix: use_response failed for {:?}",
                    hex::encode(&best_parent[..8])
                );
            } else {
                tracing::debug!(
                    "fix: no response for best_parent {:?}",
                    hex::encode(&best_parent[..8])
                );
            }
        ...
        for peer_key in peer_keys {
            ...
            let anns: Vec<wire::Announce> = to_send
                ...
                .filter_map(|k| {
                    self.infos
                        .get(k)
                        .map(|info| info.get_announce(*k).to_wire())
                })
                .collect();
    ...
    pub fn lookup(&mut self, path: &[PeerPort], watermark: &mut u64) -> Option<PeerId> {
        ...
        if self_dist >= *watermark {
            tracing::debug!(
                "Lookup path {:?} - self too far (dist={} >= watermark={})",
                path,
                self_dist,
                watermark
            );
        ...
        *watermark = self_dist;
        ...
        for k in &peer_keys {
            ...
            tracing::trace!(
                "  Peer {:02x?} dist={} (closer={}) best_dist={}",
                hex::encode(&k[..8]),
                dist,
                dist < best_dist,
                best_dist
    ...
    pub fn send_traffic(&mut self, mut tr: crate::traffic::TrafficPacket) -> Vec<RouterAction> {
        ...
        if !self
            .pathfinder
            .should_throttle_rumor(&xform, self.path_throttle)
        {
    ...
    pub fn route_traffic(&mut self, tr: crate::traffic::TrafficPacket) -> Vec<RouterAction> {
        ...
        if let Some(peer_id) = self.lookup(&path, &mut watermark) {
            ...
            tracing::debug!(
                "Traffic arrived for us: {} bytes from {:?}",
                tr.payload.len(),
                hex::encode(&tr.source[..4])
            );
            ...
            self.pathfinder
                .learn_path_from_traffic(&tr.source, &tr.from);
    ...
    fn do_send_lookup(&mut self, dest: &PublicKey) -> Vec<RouterAction> {
        if self
            .pathfinder
            .should_throttle_lookup(dest, self.path_throttle)
        {
        ...
        let n_sent = actions
            .iter()
            .filter(|a| matches!(a, RouterAction::SendPathLookup { .. }))
            .count();
        tracing::debug!(
            "PathLookup SENT dest={} coords={:?} targets={}",
            hex::encode(dest),
            from,
            n_sent
        );
    ...
    pub fn handle_lookup(
        ...
        tracing::debug!(
            "Received lookup from {:?} for dest {:?} (source={:?})",
            hex::encode(&peer_key[..8]),
            hex::encode(&lookup.dest[..8]),
            hex::encode(&lookup.source[..8])
        );
        if !self.blooms.is_on_tree(peer_key) {
            tracing::debug!(
                "Dropping lookup from {:?}: peer not on tree",
                hex::encode(&peer_key[..8])
            );
            return Vec::new();
        }
    ...
    fn handle_notify_internal(
        ...
        let mut actions = Vec::new();
        tracing::debug!(
            "PathNotify: src={} dest={} path_len={}",
            hex::encode(&notify.source[..8]),
            hex::encode(&notify.dest[..8]),
            notify.path.len()
        );
        ...
        if let Some(peer_id) = self.lookup(&notify.path, &mut watermark) {
            tracing::debug!(
                "PathNotify FWD src={} dest={} to peer={}",
                hex::encode(&notify.source[..8]),
                hex::encode(&notify.dest[..8]),
                peer_id
            );
        ...
        if notify.dest != self.crypto.public_key {
            tracing::debug!(
                "PathNotify not for us (dest={}), discarding",
                hex::encode(&notify.dest[..8])
            );
            return actions;
        }
        tracing::debug!(
            "PathNotify RECEIVED from source={} path={:?}",
            hex::encode(&notify.source[..8]),
            notify.info.path
        );
        ...
        if !Crypto::verify(&notify.source, &info_bytes, &notify.info.sig) {
            tracing::warn!(
                "PathNotify signature verification failed for source {:?}",
                hex::encode(&notify.source[..4])
            );
            return actions;
        }
    ...
    fn do_broken(&mut self, tr: &crate::traffic::TrafficPacket) -> Vec<RouterAction> {
        tracing::debug!(
            "route_traffic: no next-hop for path={:?} to {}, marking broken",
            tr.path,
            hex::encode(&tr.dest[..8])
        );
    ...
    fn handle_broken_internal(&mut self, broken: &wire::PathBroken) -> Vec<RouterAction> {
        ...
        if let Some(peer_id) = self.lookup(&broken.path, &mut watermark) {
            tracing::debug!(
                "PathBroken: forwarding source={} dest={} to peer {}",
                hex::encode(&broken.source[..8]),
        ...
        if broken.source != self.crypto.public_key {
            tracing::debug!(
                "PathBroken: discarding (not for us) source={} dest={} path={:?}",
                hex::encode(&broken.source[..8]),
                hex::encode(&broken.dest[..8]),
                broken.path
            );
        ...
        tracing::debug!(
            "PathBroken: our path to {} is broken, re-looking up",
            hex::encode(&broken.dest[..8])
        );
        ...
        if !self
            .pathfinder
            .should_throttle_lookup(&broken.dest, self.path_throttle)
        {
    ...
    pub fn handle_broken(&mut self, broken: &wire::PathBroken) -> Vec<RouterAction> {
        tracing::debug!(
            "PathBroken received: source={} dest={} path={:?}",
            hex::encode(&broken.source[..8]),
            hex::encode(&broken.dest[..8]),
            broken.path
        );
    ...
    fn best_peer_for_key(&self, key: &PublicKey) -> Option<PeerId> {
        self.peers
            .get(key)
            .and_then(|peers| peers.values().min_by_key(|e| e.prio).map(|e| e.id))
    }
    ...
    pub fn expire_infos(&mut self) {
        ...
        for key in expired {
            if key == self_key {
                tracing::debug!(
                    "expire_infos: self info aged out, triggering refresh (router_refresh={}s)",
                    self.router_refresh.as_secs()
                );
                self.refresh = true;
            } else {
                tracing::debug!(
                    "expire_infos: evicting node {} (last seen {}s ago)",
                    hex::encode(&key[..8]),
                    self.info_times
                        .get(&key)
        ...
        if removed > 0 {
            tracing::debug!(
                "expire_infos: evicted {} nodes, tree now has {} entries (timeout={}s)",
                removed,
                self.infos.len(),
                self.router_timeout.as_secs()
            );
        }
    }
