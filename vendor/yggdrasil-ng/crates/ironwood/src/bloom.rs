...
impl Blooms {
    ...
    pub fn is_on_tree(&self, key: &PublicKey) -> bool {
        self.blooms.get(key).map_or(false, |info| info.on_tree)
    }
...
pub fn decode_bloom(data: &[u8]) -> Result<[u64; BLOOM_FILTER_U64S], BloomError> {
    ...
    let flags0: [u8; BLOOM_FILTER_FLAGS] = flags0
        .try_into()
        .map_err(|_| BloomError::Decode("Invalid flags0"))?;
    let flags1: [u8; BLOOM_FILTER_FLAGS] = flags1
        .try_into()
        .map_err(|_| BloomError::Decode("Invalid flags1"))?;
        ...
        if f0 != 0 && f1 != 0 {
            ...
            if byte_idx + 8 > rest.len() {
                return Err(BloomError::Decode(
                    "Not enough data for non-compressed chunk",
                ));
            }
...
mod tests {
    ...
    fn test_known_values_single_key() {
        ...
        assert!(
            expected_filter.equal(&filter),
            "Filter not equals the expected filter!"
        );

        // Print raw bitset
        let raw = filter.as_raw();
        let non_zero: Vec<_> = raw
            .iter()
            .enumerate()
            .filter(|&(_, v)| *v != 0)
            .map(|(i, v)| (i, *v))
