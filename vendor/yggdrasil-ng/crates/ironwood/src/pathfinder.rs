...
impl Pathfinder {
    ...
    pub fn should_throttle_rumor(&self, xformed_dest: &PublicKey, throttle: Duration) -> bool {
    ...
    pub fn accept_notify(
        ...
        let traffic = self.rumors.get_mut(&xformed_source).and_then(|rumor| {
            if rumor.traffic.as_ref().map_or(false, |t| t.dest == source) {
                rumor.traffic.take()
            } else {
                None
            }
    ...
    pub fn cache_traffic(&mut self, dest: &PublicKey, traffic: super::traffic::TrafficPacket) {
        if let Some(info) = self.paths.get_mut(dest) {
    ...
    pub fn cleanup_expired(&mut self, path_timeout: Duration) {
        let now = Instant::now();
        self.paths
            .retain(|_, info| now.duration_since(info.last_refresh) < path_timeout);
...
mod tests {
    ...
    fn learn_path_only_fills_empty_slot() {
        ...
        assert_eq!(
            pf.paths[&src].path,
            vec![1, 2, 3],
            "must not overwrite a known path"
        );
        ...
        assert!(
            pf.paths[&src].broken,
            "must not un-break via unsigned traffic"
        );
        assert_eq!(pf.paths[&src].path, vec![1, 2, 3]);
    }
