...
use ironwood::{
    Config, PacketConn, PacketConnImpl, new_encrypted_packet_conn, new_packet_conn,
    new_signed_packet_conn,
};
...
async fn two_node_bidirectional() {
    ...
    assert!(
        rb.expect("timeout B").expect("panic B"),
        "B never got msg from A"
    );
    assert!(
        ra.expect("timeout A").expect("panic A"),
        "A never got msg from B"
    );
...
async fn three_node_chain() {
    ...
    assert!(
        result.expect("timeout").expect("panic"),
        "C never got msg from A"
    );
...
async fn cross_hub_forward_discovery_retry() {
    ...
    for trial in 0..trials {
        ...
        assert!(
            ok,
            "trial {}: D did not receive the packet within 8s",
            trial
        );
    }
}
...
async fn silent_peer_is_probed_before_disconnect() {
    ...
    async fn time_to_disconnect(probes: u32) -> Duration {
        ...
        let peer_addr =
            ironwood::Addr::from(SigningKey::generate(&mut OsRng).verifying_key().to_bytes());
