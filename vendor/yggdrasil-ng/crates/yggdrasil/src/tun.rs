...
impl TunAdapter {
    ...
    pub async fn new(
        ...
        if let Some(ckr_cfg) = ckr_config {
            if ckr_cfg.enable
                && !ckr_cfg.ipv4_address.is_empty()
                && ckr_cfg.ip_addresses.iter().all(|s| s.is_empty())
            {
                let (v4_addr, v4_prefix) = parse_ipv4_cidr(&ckr_cfg.ipv4_address)?;
                builder = builder.ipv4(v4_addr, v4_prefix, None);
                tracing::info!(
                    "CKR: assigning IPv4 address {} to TUN",
                    ckr_cfg.ipv4_address
                );
            }
        }
                        ...
                        if parts.len() == 1 || parts.len() == 2 {
                            ...
                            let prefix: u8 = if parts.len() == 1 {
                                ...
                                parts[1].parse().map_err(|e| {
                                    format!("invalid IPv6 prefix in ip_addresses '{}': {}", cidr, e)
                                })?
                            };
                            let ip: Ipv6Addr = ip_str.parse().map_err(|e| {
                                format!("invalid IPv6 in ip_addresses '{}': {}", cidr, e)
                            })?;
                            builder = builder.ipv6(ip, prefix);
                            tracing::info!("CKR: assigning IPv6 address {} to TUN", cidr);
                        } else {
                            return Err(format!(
                                "invalid IPv6 CIDR in ip_addresses '{}': expected addr or addr/prefix",
                                cidr
                            ));
                        }
                    } else {
        ...
        if !dns_servers.is_empty() {
            if is_set_interface_dns_settings_supported() {
                match set_interface_dns(dns_servers) {
                    Ok(()) => tracing::info!(
                        "Set DNS servers on TUN interface: {}",
                        dns_servers.join(", ")
                    ),
...
fn parse_ipv4_cidr(cidr: &str) -> Result<(Ipv4Addr, u8), String> {
    ...
    let (addr_str, prefix_str) = if parts.len() == 1 {
        ...
        return Err(format!(
            "invalid IPv4 CIDR '{}': expected addr or addr/prefix",
            cidr
        ));
    };
...
fn set_interface_registration(guid: windows::core::GUID, enabled: bool) -> Result<(), String> {
    use windows::Win32::NetworkManagement::IpHelper::{
        DNS_INTERFACE_SETTINGS, DNS_INTERFACE_SETTINGS_VERSION1, DNS_SETTING_REGISTRATION_ENABLED,
    };
