...
use crate::address::{
    Address, Subnet, addr_for_key, is_valid_address, is_valid_subnet, subnet_for_key,
};
use crate::core::Core;
use crate::firewall::Firewall;
#[cfg(feature = "ckr")]
use {crate::ckr::CryptoKey, crate::config::TunnelRoutingConfig, std::net::IpAddr};
...
impl ReadWriteCloser {
    ...
    pub async fn read<'a>(&self, buf: &'a mut [u8]) -> Result<&'a [u8], String> {
        ...
        let range = loop {
            ...
            tracing::debug!(
                "RWC read {} bytes from {:?}, first byte={:#x}",
                n,
                from_addr,
                packet[0]
            );
            ...
            if let Some(ref ckr) = self.ckr {
                let accepted = self
                    .ckr_read_validate(ckr, packet, is_ip4, is_ip6, &from_key)
                    .await;
                if !accepted {
                    continue;
                }
            ...
            if !dst_is_addr && !dst_is_subnet {
                tracing::debug!(
                    "RWC dropping: dst {:x?} is neither our addr nor subnet",
                    &dst_ip[..4]
                );
                continue;
            }
    ...
    pub async fn write(&self, buf: &[u8]) -> Result<usize, String> {
        ...
        if let Some(key) = key {
            ...
            tracing::debug!(
                "RWC write: key unknown {}, buffering + lookup",
                Address(dst_ip)
            );
