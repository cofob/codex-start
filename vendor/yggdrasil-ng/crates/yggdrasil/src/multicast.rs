...
fn discover_interfaces(
    ...
    for entry in entries {
        ...
        if !flags.contains(InterfaceFlags::MULTICAST) {
            continue;
        }
        if flags.contains(InterfaceFlags::POINTTOPOINT) {
            continue;
        }
                    ...
                    if let Some(link_local) = state.addrs.first() {
                        ...
                        .await
                        {
                            Ok((actual_port, lcancel, handle)) => {
                                tracing::info!(
                                    "Multicast beacon listener on {} port {}",
                                    name,
                                    actual_port
                                );
                                lsns.insert(
                            ...
                            Err(e) => {
                                tracing::warn!(
                                    "Failed to start multicast listener on {}: {}",
                                    name,
                                    e
                                );
                            }
                ...
                if state.beacon {
                    if let Some(listener_info) = lsns.get(name) {
                        if state.last_beacon.elapsed() >= state.beacon_interval {
                            ...
                            let dest_with_scope =
                                SocketAddrV6::new(*dest.ip(), dest.port(), 0, state.index);
                            if let Err(e) = udp.send_to(&msg, dest_with_scope).await {
                                tracing::debug!(
                                    "Failed to send multicast beacon on {}: {}",
                                    name,
                                    e
                                );
                            }
...
async fn receiver_loop(
    ...
    loop {
        ...
        let (password, priority, iface_name) = {
            ...
            let found = ifaces
                .iter()
                .find(|(_, state)| state.index == scope_id && state.listen);
            match found {
                Some((name, state)) => {
                    // Verify auth hash
        ...
        let uri = format!("tls://[{}%{}]:{}", from_v6.ip(), iface_name, adv.port,);
...
async fn connect_to_peer(
    ...
    let server_name =
        rustls::pki_types::ServerName::IpAddress(std::net::IpAddr::V6(*addr.ip()).into());
