...
mod tests {
    ...
    async fn test_ws_server_requires_subprotocol() {
        ...
        let (client_io, server_io) = duplex_pair();
        let server =
            tokio::spawn(async move { ws_server_handshake(server_io, dummy_addr(), None).await });
        ...
        assert!(
            ...
            "client without subprotocol must be rejected"
        );
    ...
    async fn test_ws_message_size_limit() {
        ...
        let writer = tokio::spawn(async move {
            ...
            let _ = raw_client
                .send(Message::Binary(Bytes::from(oversized)))
                .await;
        });
