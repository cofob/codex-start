...
pub async fn run_ctl(
    ...
    let stream = TcpStream::connect(addr)
        .await
        .map_err(|e| format!("Failed to connect to admin socket at {}: {}", endpoint, e))?;
                ...
                if peers.is_empty() {
                    ...
                    let rows: Vec<Vec<String>> = peers
                        ...
                        .map(|peer| {
                            ...
                            vec![
                                uri, state, dir, address, latency, cost, uptime, rx_bytes,
                                tx_bytes, rx_rate, tx_rate, priority, last_error,
                            ]
                        })
                        .collect();
...
fn format_table(
    ...
    if let (Some(flex), Some(term)) = (flex_col, term_width) {
        ...
        if total > term {
            let shrunk = widths[flex]
                .saturating_sub(total - term)
                .max(MIN_FLEX_WIDTH);
...
mod tests {
    ...
    fn sample() -> (Vec<&'static str>, Vec<Vec<String>>) {
        ...
        let rows = vec![
            vec![
                "tls://[201:23b0:9e11:2a7c:4d5f::1]:7743".into(),
                "Up".into(),
                "21.0ms".into(),
            ],
            vec![
                "tcp://192.168.44.125:7743".into(),
                "Up".into(),
                "1.1ms".into(),
            ],
        ];
