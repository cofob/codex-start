...
async fn async_main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    opts.optflagopt(
        "g",
        "genconf",
        "Generate a new configuration (optionally save to FILE)",
        "FILE",
    );
    ...
    opts.optflag(
        "",
        "service",
        "Run as a Windows service (launched by the Service Control Manager)",
    );
    ...
    let config_path = matches
        .opt_str("config")
        .unwrap_or_else(|| "yggdrasil.toml".to_string());
    ...
    let signing_key = if !config.private_key.is_empty() {
        ...
        let key_bytes: [u8; 64] = bytes.try_into().map_err(|v: Vec<u8>| {
            format!("YGGDRASIL_PRIVATE_KEY should be 64 bytes, got {}", v.len())
        })?;
...
fn print_ctl_commands() {
    ...
    println!(
        "    list, getSelf, getPeers, getTree, getPaths, getSessions, getTUN, getMulticastInterfaces"
    );
    println!("  Debug:");
