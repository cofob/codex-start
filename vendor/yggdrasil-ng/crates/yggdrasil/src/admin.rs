...
async fn handle_request(req: &AdminRequest, core: &Arc<Core>) -> Result<serde_json::Value, String> {
    match req.request.to_lowercase().as_str() {
            ...
            let peers_json: Vec<serde_json::Value> = peers
                ...
                .map(|p| {
                    let (address, subnet, key) = if p.up {
                        (
                            addr_for_key(&p.key).to_string(),
                            subnet_for_key(&p.key).to_string(),
                            hex::encode(p.key),
                        )
                    } else {
        ...
        "getdebug" => {
            ...
            let peer_latencies: Vec<serde_json::Value> = snap
                .peer_latencies_ms
                .iter()
                .map(|(key, ms)| {
            ...
            let pending_lookups: Vec<serde_json::Value> = snap
                .pending_lookups
                .iter()
                .map(|p| {
        ...
        "getlookup" => {
            ...
            let key_bytes = hex::decode(key_str).map_err(|_| "invalid hex key")?;
            if key_bytes.len() != 32 {
        ...
        "debug_remotegetpeers" => {
            ...
            let peer_keys = core
                .proto_handler()
                .send_get_peers_request(key)
                .await
                .map_err(|e| format!("debug_remoteGetPeers failed: {}", e))?;

            let keys_json: Vec<String> = peer_keys.iter().map(|k| hex::encode(k)).collect();
        ...
        "debug_remotegettree" => {
            ...
            let tree_keys = core
                .proto_handler()
                .send_get_tree_request(key)
                .await
                .map_err(|e| format!("debug_remoteGetTree failed: {}", e))?;

            let keys_json: Vec<String> = tree_keys.iter().map(|k| hex::encode(k)).collect();
        ...
        other => Err(format!("unknown action '{}', try 'list' for help", other)),
    }
}
