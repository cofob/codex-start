...
pub fn proto_needs(payload: &[u8]) -> ProtoNeeds {
    ...
    match payload.get(1) {
        Some(&TYPE_DEBUG_GET_SELF_REQUEST) => ProtoNeeds {
            routing_entries: true,
            ..needs
        },
        Some(&TYPE_DEBUG_GET_PEERS_REQUEST) => ProtoNeeds {
            peer_keys: true,
            ..needs
        },
        Some(&TYPE_DEBUG_GET_TREE_REQUEST) => ProtoNeeds {
            tree_keys: true,
            ..needs
        },
        _ => needs,
...
impl ProtoHandler {
    ...
    async fn handle_nodeinfo_request(
        &self,
        from_key: [u8; 32],
        nodeinfo_json: &str,
    ) -> Option<(Addr, Vec<u8>)> {
    ...
    pub async fn send_get_self_request(&self, target_key: [u8; 32]) -> Result<JsonValue, String> {
        ...
        match tokio::time::timeout(REQUEST_TIMEOUT, rx).await {
            Ok(Ok(data)) => {
                serde_json::from_slice(&data).map_err(|e| format!("Invalid JSON response: {}", e))
            }
    ...
    async fn cleanup_expired(&self) {
        ...
        self.self_callbacks
            .lock()
            .await
            .retain(|_, (_, created)| now.duration_since(*created) < CALLBACK_TIMEOUT);

        self.peers_callbacks
            .lock()
            .await
            .retain(|_, (_, created)| now.duration_since(*created) < CALLBACK_TIMEOUT);
