...
impl Metadata {
    ...
    pub fn decode<R: Read>(reader: &mut R, password: &[u8]) -> Result<Self, VersionError> {
        ...
        let verifying_key =
            VerifyingKey::from_bytes(&meta.public_key).map_err(|_| VersionError::InvalidKey)?;
        verifying_key
...
fn blake2b_hash(data: &[u8], password: &[u8]) -> [u8; 64] {
    if password.is_empty() {
        ...
        let mut mac =
            Blake2bMac512::new_from_slice(password).expect("BLAKE2b key length should be valid");
        mac.update(data);
