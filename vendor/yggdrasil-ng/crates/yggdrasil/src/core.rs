...
use crate::address::{Address, Subnet, addr_for_key, subnet_for_key};
use crate::config::Config;
use crate::ipv6rwc::ReadWriteCloser;
use crate::links::{ActiveLinks, LinkPeerInfo, Links};
...
impl Core {
    ...
    pub async fn read_from(&self, buf: &mut [u8]) -> Result<(Range<usize>, Addr), ironwood::Error> {
        loop {
    ...
    async fn handle_inbound_proto(&self, from_key: [u8; 32], payload: &[u8]) {
        ...
        if let Some((target, response)) = self
            .proto_handler
            .handle_proto_message(
                from_key,
                payload,
                &our_key,
                routing_entries,
    ...
    pub fn subscribe_peer_events(
        &self,
    ) -> tokio::sync::broadcast::Receiver<crate::links::PeerEvent> {
    ...
    async fn tls_renewal_task(self: Arc<Self>) {
        loop {
            ...
            if days_until_expiry <= 10 {
                tracing::info!(
                    "TLS certificate expires in {} days, renewing...",
                    days_until_expiry
                );
                ...
                match tls::generate_self_signed_cert(&self.signing_key) {
                    Ok(material) => {
                        ...
                        match (server_result, client_result) {
                            ...
                            (Err(e), _) | (_, Err(e)) => {
                                tracing::error!(
                                    "Failed to create TLS config during renewal: {}",
                                    e
                                );
                            }
                        }
