...
impl ActiveLinks {
    ...
    async fn register(
        ...
        key: [u8; 32],
        priority: u8,
    ) -> Option<(u64, Arc<AtomicUsize>, Arc<AtomicUsize>)> {
        let mut inner = self.inner.lock().await;
        // Reject duplicate: same key + same direction
        if inner
            .connections
            .values()
            .any(|c| c.key == key && c.inbound == inbound)
        {
            return None;
        }
        ...
        let _ = self
            .peer_tx
            .send(PeerEvent::Connected { key, uri, inbound });
        Some((id, rx, tx))
    }
    ...
    pub async fn update_rates(&self) {
        ...
        for conn in inner.connections.values_mut() {
            ...
            conn.rx_rate
                .store(rx.saturating_sub(conn.last_rx), Ordering::Relaxed);
            conn.tx_rate
                .store(tx.saturating_sub(conn.last_tx), Ordering::Relaxed);
            conn.last_rx = rx;
            conn.last_tx = tx;
        }
    }
...
impl Links {
    ...
    pub async fn add_peer(&mut self, uri: &str) -> Result<(), String> {
        ...
        let addr_key: Option<String> = match tokio::net::lookup_host(&target).await {
            Ok(addrs) => {
                ...
                for addr in &resolved {
                    ...
                    if let Some(existing_uri) = self.peer_addrs.get(&ak) {
                        return Err(format!(
                            "peer {} already connected as {} (resolves to same address {})",
                            uri, existing_uri, ak
                        ));
                    }
                }
            ...
            Err(e) => {
                tracing::warn!(
                    "DNS lookup failed for {} ({}), skipping duplicate check — will retry in reconnect loop",
                    target,
                    e
                );
                None
                        ...
                        match result {
                            ...
                            Err(e) => {
                                peer_errors
                                    .lock()
                                    .await
                                    .insert(uri_str.clone(), Some(e.to_string()));
                            }
                        }
                    }
                    Err(err_msg) => {
                        ...
                        peer_errors
                            .lock()
                            .await
                            .insert(uri_str.clone(), Some(err_msg));
                    }
                }
        ...
        self.peers
            .insert(uri.to_string(), PeerEntry { cancel, handle });
...
pub(crate) async fn handle_connection(
    ...
    if !remote_meta.check() {
        ...
        if let Some(ip) = peer_ip {
            ...
            active
                .ban_list
                .record_failure(ip, "incompatible version")
                .await;
        } else {
    ...
    if !options.pinned_keys.is_empty() && !options.pinned_keys.contains(&remote_meta.public_key) {
        if let Some(ip) = peer_ip {
...
async fn dial_stream(
    ...
    if let Some(connector) = tls_connector {
        ...
        let server_name = rustls::pki_types::ServerName::try_from(sni_host).unwrap_or_else(|_| {
...
mod tests {
    ...
    fn test_parse_duration_string_plain_seconds() {
        ...
        assert_eq!(
            parse_duration_string("4096").unwrap(),
    ...
    fn test_parse_duration_string_units() {
        ...
        assert_eq!(
            parse_duration_string("2h30m15s").unwrap(),
