...
- `host`: engine host networking. Declared and requested port publications are ignored because host and container share the network namespace; native MCP OAuth uses that shared loopback directly or a host-loopback alias relay when its safe callback URL differs from Codex's IPv4 listener.
