...
VS Code setup changes only `chatgpt.cliExecutable`. It preserves JSONC comments and other settings. Desktop setup writes `~/Library/LaunchAgents/cs.fob.wtf.adapter.plist` and sets `CODEX_CLI_PATH` in the login environment. The LaunchAgent restores this value at login. It does not modify the application bundle. Quit and reopen Desktop, or reload the VS Code window, to use the adapter.
...
- Desktop and IDE `fs/*` requests for host paths run on the host. They do not start project containers. This includes attachments, visualizations, and file watches. These RPCs come from the client connection; workload tools still run in their container. Paths below `/home/codex` retain their container meaning and use the common server.
- The adapter uses the native Codex executable from the active Desktop or IDE client for initialization and catalogue requests. These requests include configuration reads, plugins, skills, hooks, and adding a Desktop marketplace from a host path. The native process uses the selected catalogue home, but `CODEX_SQLITE_HOME` points to an isolated temporary directory. It can use host marketplace and configuration paths without opening container SQLite databases through a host bind mount. The adapter does not use native Codex for saved thread state because container rollouts contain `/home/codex` paths that do not exist on the host. One lazily started generic control container serves thread listing, loading, and resume requests for all saved projects. Client startup can therefore start at most this one control container instead of one container for each project. The adapter reuses the control container across a request burst. It stops it after at most 30 seconds without state activity, even when threads were loaded. The normal adapter idle timeout applies when it is shorter. A new thread starts directly in its project container because its rollout file does not exist before its first turn. Existing control-loaded threads start a project container only when work starts, such as `turn/start`, compaction, or a shell command. Before the first turn for a control-loaded thread, the adapter stops the control container to release its writer lock, then resumes the shared rollout inside the project container. After a project container exists, read-only requests for its `threadId` or single `cwd` also use that project container. Native catalogue reads are used only when no matching project container exists and the project uses the same Codex home. The adapter maps skill paths in results back to `/home/codex`, so a later container can use the selected skill. Set `CODEX_START_ADAPTER_HOST_CODEX` to an executable path to select it explicitly, or set it to `off` to keep all app-server requests in containers.
- Before the adapter forwards user input, it copies each client attachment outside the project directly into the running project container. This includes structured `localImage`, `localAudio`, `mention`, and `skill` input and the file blocks that Desktop and IDE clients add to text input. Files and directories keep the same absolute path. The adapter does not rewrite attachment paths. It copies each path only once for the lifetime of that project server; later host changes are not synchronized. When a project server is recreated, the adapter reads attachment paths from the thread rollout and copies all still-existing attachments before it resumes the thread. Existing container paths are not overwritten. Files that are already inside the mounted project need no copy.
- Catalogue requests without a project, such as initialization, account information, and model lists, use the native process. Saved thread state and requests that must stay sandboxed use one generic server in a temporary directory, which starts only when needed. Its Codex home persists, and its scratch directory is removed when the client exits. New execution requests must specify `cwd`; they cannot start tasks in this temporary directory. CLI probes such as `--version` also use a scratch directory, so a GUI process started at `/` cannot cause the adapter to mount the host root.
...
Each container mounts its project at the same canonical absolute path as the host. A linked worktree also mounts its common Git metadata. The adapter does not create another worktree. ChatGPT desktop app worktrees can use detached HEAD and can be outside the codex-start worktree directory. `codex-start worktree list` shows them as external registered worktrees. Lifecycle commands can select them, but `worktree cleanup` leaves them for ChatGPT to manage. Project and global settings select the environment, network policy, resources, secrets, and Codex home. Set common defaults through `codex-start config`, its global configuration file, or `CODEX_START__...` environment variables. Adapter arguments belong to Codex, not the launcher.

The client owns the server processes. The adapter disables TTY allocation, update prompts, and persistent-session management. Container preparation gets no stdin, and its output goes to stderr. The foreground runner handles SIGINT and SIGTERM. When the common client closes, the adapter closes and stops its project servers. A project server with no loaded threads, active turns, pending requests, or approval replies stops after five idle minutes. Before it stops the container, the adapter reads its process list. A live process other than Codex and the managed `codex-start-init` services keeps the container running. This includes sleeping processes and TCP or UDP servers. Zombie processes do not keep it running. After the last external process exits, a new full idle period starts. If process inspection fails, the adapter preserves the container and reports one warning for the continuous failure. Closing or unsubscribing from a thread releases its server when no other work uses it. Metadata-only project servers also expire. Loaded threads remain available between turns; they are not stopped just because the user pauses. The process check does not extend the control-container lifetime. One common server remains for the client connection. Host file watches use one-second polling and report changed paths through `fs/changed`.
...
The default is five minutes. The minimum is one second. `CODEX_START__ADAPTER__IDLE_TIMEOUT_SECONDS` overrides the config value. The adapter reads this connection-wide setting at startup, before selecting any project. Restart the Desktop app or reload the IDE window after changing it. Project settings do not change this connection-wide timeout. Client shutdown and project-server failure still stop the container even if another process is running.
